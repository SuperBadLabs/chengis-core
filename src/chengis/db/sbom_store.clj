...
;; create-sbom!  [ds attrs]              — overridden below (custom row defaults)
;; get-sbom      [ds id & {:keys [org-id]}]
;; list-sboms    [ds & {:keys [org-id]}] — overridden below (job-id, limit, offset)
;; update-sbom!  [ds id updates]
;; delete-sbom!  [ds id & {:keys [org-id]}]
;; count-sboms   [ds & {:keys [org-id]}]
;; ---------------------------------------------------------------------------
...
;; ---------------------------------------------------------------------------
;; Override: create — custom row shape with numeric defaults
;; ---------------------------------------------------------------------------
...
(defn create-sbom!
  ...
  (let [id  (or id (util/generate-id))
    ...
    (jdbc/execute-one! ds
                       (sql/format {:insert-into :sbom-reports :values [row]})
                       {:builder-fn rs/as-unqualified-kebab-maps})
    (log/info "Created SBOM report" {:id id :build-id build-id :format sbom-format})
    row))

;; ---------------------------------------------------------------------------
;; Override: list — adds job-id, limit, offset filters
;; ---------------------------------------------------------------------------
