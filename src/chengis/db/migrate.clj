...
(defn migration-config
  "Build migratus configuration for a database.
   ...
   Recognized config-map keys:
     :type            \"sqlite\" (default) | \"postgresql\"
     :path            SQLite file path (sqlite only)
     :host / :port    Postgres host/port (postgresql only)
     :dbname          Postgres database name (postgresql only)
     :user / :password Postgres credentials (postgresql only)
     :migrations-dir  Override migration directory. Defaults to
                      \"migrations/sqlite\" / \"migrations/postgresql\".
                      Consuming products (anvil, future GitLab compat, …)
                      supply their own directory under their resources/."
  [db-config]
  (if (string? db-config)
    ...
    (case (get db-config :type "sqlite")
      "sqlite"
      {:store :database
       :migration-dir (get db-config :migrations-dir "migrations/sqlite")
       :db {:dbtype "sqlite" :dbname (get db-config :path "chengis.db")}}

      "postgresql"
      {:store :database
       :migration-dir (get db-config :migrations-dir "migrations/postgresql")
...
(defn migrate!
  ...
  [db-config]
  ;; migratus.core/migrate is a macro that expands to a single-form do;
  ;; ignore kondo's redundant-do at the call site.
  #_:clj-kondo/ignore
