...
(defn- builds-handler
  "POST /builds — Receive a build dispatch from the master.
   Validates auth (HMAC or Bearer) before accepting the build.
   Fail-closed in production mode when no auth-token is configured."
  [agent-config req]
  ...
  (let [body-str (try (slurp (:body req)) (catch Exception _ nil))
        ...
        auth-scheme (or (:auth-scheme agent-config) :hmac)
        fail-closed? (= "production" (:environment agent-config))]
    (if-not (agent-auth/verify-agent-request
             auth-scheme (:auth-token agent-config) req body-bytes
             {:fail-closed? fail-closed?})
      {:status 401
       :headers {"Content-Type" "application/json"}
       :body (json/write-str {:error "Unauthorized"})}
      (let [body (when body-str
...
(defn start-agent!
  ...
  (let [port (or port 9090)
    ...
    ;; Warn if auth token is not configured
    (when-not auth-token
      (log/error "SECURITY: Agent started without auth-token — /builds endpoint is UNAUTHENTICATED!")
      (log/error "Set --auth-token or CHENGIS_DISTRIBUTED_AUTH_TOKEN for production use."))
    ...
    (let [agent-id (client/register-with-master!
      ...
      (let [agent-config (assoc config :agent-id agent-id)]
        ...
        (let [tls-config (get-in config [:tls])
              server-opts (cond-> {:port port}
                            (:keystore tls-config)
                            (assoc :ssl? true
                                   :keystore (:keystore tls-config)
                                   :key-password (:keystore-password tls-config)))
