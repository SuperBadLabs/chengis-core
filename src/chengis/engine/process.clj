...
(defn execute-command
  ...
  [{:keys [command dir env timeout mask-values]
    :or {timeout 300000}}]
  ;; Mask secret values out of every LOGGED command string when callers opt
  ;; in via :mask-values. Without this, any caller that embeds a credential
  ;; in the shell command (e.g. an HTTPS PAT for git) leaks it on the first
  ;; log line — even though we mask :stdout/:stderr later (Codex PR #83 r1
  ;; P1). The shell still runs the un-masked command; only the log view is
  ;; redacted.
  (let [logged-command (cond-> command
                         (seq mask-values) (masker/mask-secrets mask-values))]
    (log/info "Executing command:" logged-command (when dir (str "(in " dir ")")))
    (let [start-time (System/currentTimeMillis)
        ...
        ;; Access the underlying java.lang.Process for timeout support
          ^Process java-proc (:proc proc)]
      (try
        (let [timed-out? (when timeout
                           (not (.waitFor java-proc timeout TimeUnit/MILLISECONDS)))
              _ (when timed-out?
                  (log/warn "Command timed out after" timeout "ms:" logged-command)
                ...
                ;; Force kill if still alive after grace period
                  (when-not (.waitFor java-proc 5 TimeUnit/SECONDS)
                    (log/warn "Force killing process:" logged-command)
        ...
        (catch InterruptedException _
          (log/warn "Command interrupted (build cancelled):" logged-command)
          (try (bp/destroy-tree proc)
               (catch Exception e
