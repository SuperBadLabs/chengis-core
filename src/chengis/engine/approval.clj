(ns chengis.engine.approval
   ...
   *parked* (not blocked) while waiting. This prevents approval gates from
   ...
   Blocking JDBC calls inside the poll go-loop are offloaded via
   `(async/<! (async/thread ...))` so a slow SQL Socket.read can't starve
   the small (8-thread) async-dispatch pool that runs every go-block in the
   process.
...
(defn- gate-timed-out?
  ...
  (when-let [timeout-minutes (:timeout-minutes gate)]
    (let [^Instant created-at (parse-datetime (:created-at gate))
          deadline (.plus created-at (Duration/ofMinutes (long timeout-minutes)))
          now (Instant/now)]
...
(defn wait-for-approval!
  ...
  (let [poll-ms (or poll-interval-ms 5000)
    ...
    (go-loop []
      ...
      (let [[_ _port] (alts! [notify-ch (timeout poll-ms)])]
        ;; Check cancellation
        (if (and cancelled? @cancelled?)
          (do
            (log/info "Approval wait cancelled for gate" gate-id)
            ;; JDBC is blocking — offload to the cached thread pool so the
            ;; async-dispatch pool isn't starved by a Socket.read on SQLite.
            (async/<! (async/thread
                        (try (approval-store/reject-gate! ds gate-id "system:cancelled")
                             (catch Exception _))))
            (deliver result-promise
                     {:approved false :reason "Build cancelled during approval wait"}))
          ;; Check gate status — JDBC call offloaded to the thread pool.
          (let [gate (async/<! (async/thread (approval-store/get-gate ds gate-id)))]
            (case (:status gate)
              "approved"
              ...
              (if (gate-timed-out? gate)
                (do (async/<! (async/thread (approval-store/timeout-gate! ds gate-id)))
