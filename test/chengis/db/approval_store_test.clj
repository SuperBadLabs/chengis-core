(ns ^:integration chengis.db.approval-store-test
...
;; ---------------------------------------------------------------------------
;; MUT-P2 (Pareto): can-user-respond?, count-rejections, multi-approver reject.
;; ---------------------------------------------------------------------------

(deftest can-user-respond-test
  (let [ds (conn/create-datasource test-db-path)]
    (testing "a group member who hasn't responded can respond; not after responding; outsiders cannot"
      (let [gid (approval-store/create-gate! ds {:build-id "b1" :stage-name "s"
                                                 :min-approvals 2 :approver-group ["alice" "bob"]})]
        (is (true? (approval-store/can-user-respond? ds gid "alice")))
        (approval-store/record-response! ds gid "alice" "approved")
        (is (not (approval-store/can-user-respond? ds gid "alice")))
        (is (not (approval-store/can-user-respond? ds gid "mallory")))))
    (testing "with no approver group, any user may respond to a pending gate"
      (let [gid (approval-store/create-gate! ds {:build-id "b2" :stage-name "s"})]
        (is (true? (approval-store/can-user-respond? ds gid "anyone")))))
    (testing "a resolved (non-pending) gate accepts no responses"
      (let [gid (approval-store/create-gate! ds {:build-id "b3" :stage-name "s" :min-approvals 1})]
        (approval-store/approve-gate! ds gid "approver")
        (is (not (approval-store/can-user-respond? ds gid "alice")))))))

(deftest count-rejections-test
  (let [ds (conn/create-datasource test-db-path)
        gid (approval-store/create-gate! ds {:build-id "b" :stage-name "s"
                                             :min-approvals 3 :approver-group ["a" "b" "c"]})]
    (approval-store/record-response! ds gid "a" "rejected")
    (approval-store/record-response! ds gid "b" "rejected")
    (testing "count-rejections counts only rejected responses"
      (is (= 2 (approval-store/count-rejections ds gid)))
      (is (= 0 (approval-store/count-approvals ds gid))))))

(deftest reject-gate-multi-approver-test
  (let [ds (conn/create-datasource test-db-path)]
    (testing "one rejection of a min-2-of-3 gate leaves the threshold reachable (stays pending)"
      (let [gid (approval-store/create-gate! ds {:build-id "b1" :stage-name "s"
                                                 :min-approvals 2 :approver-group ["a" "b" "c"]})]
        (is (= 0 (approval-store/reject-gate! ds gid "a")))
        (is (= "pending" (:status (approval-store/get-gate ds gid))))))
    (testing "a second rejection makes the threshold unreachable -> gate is rejected"
      (let [gid (approval-store/create-gate! ds {:build-id "b2" :stage-name "s"
                                                 :min-approvals 2 :approver-group ["a" "b" "c"]})]
        (approval-store/reject-gate! ds gid "a")
        (is (= 1 (approval-store/reject-gate! ds gid "b")))
        (is (= "rejected" (:status (approval-store/get-gate ds gid))))))))
