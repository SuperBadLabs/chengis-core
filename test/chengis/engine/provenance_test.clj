(ns ^:integration chengis.engine.provenance-test
  ...
  (testing "wrap-dsse-envelope structure validation"
    (let [payload-type "https://slsa.dev/provenance/v1"
      ...
      (let [^Base64$Decoder decoder (Base64/getDecoder)
            ^bytes decoded-bytes (.decode decoder ^String (:payload envelope))
            decoded-str (String. decoded-bytes java.nio.charset.StandardCharsets/UTF_8)
