...
;; ---- Bounded pool: thread-count regression spy (stages) ----
;;
;; Mirrors `run-steps-parallel-thread-count-bounded-test` but for the
;; DAG-driven parallel stage executor. 30 independent stages (no
;; `:depends-on`) should all be runnable at once; the bounded
;; `parallel-stage-executor` (default pool size 4) caps live threads.
;; Pre-fix, each stage allocated a fresh `core.async/thread`, so we
;; would see ~30 distinct names. Post-fix, ≤ ~6.
(deftest dag-stage-thread-count-bounded-test
  (testing "30 independent DAG stages reuse the bounded stage pool"
    (let [thread-names (java.util.concurrent.ConcurrentHashMap/newKeySet)
          exec-count (atom 0)
          ;; Build a pipeline of 30 independent stages, each with one trivial
          ;; step. All stages have `:depends-on []`, so the DAG scheduler
          ;; sees all 30 as runnable in the first wave.
          stages (mapv (fn [i]
                         (dsl/stage (str "S" i) {:depends-on []}
                                    (dsl/step (str "step-" i)
                                              (dsl/sh (str "echo " i)))))
                       (range 30))
          pipeline {:pipeline-name "dag-thread-spy"
                    :stages stages}]
      (with-redefs [process/execute-command
                    (fn [{:keys [command]}]
                      (.add thread-names (.getName (Thread/currentThread)))
                      (swap! exec-count inc)
                      ;; Sleep so concurrent stages genuinely overlap on
                      ;; different pool threads — without this, very fast
                      ;; in-flight serialisation could shrink the observed
                      ;; thread set even with the broken (pre-fix) code.
                      (Thread/sleep 10)
                      {:exit-code 0 :stdout command :stderr "" :duration-ms 10})]
        (let [result (executor/run-build base-system pipeline {})]
          (is (= :success (:build-status result)))
          (is (= 30 (count (:stage-results result))))
          (is (every? #(= :success (:stage-status %)) (:stage-results result)))
          (is (= 30 @exec-count))
          ;; Pool default is 4 — allow a small fudge.
          (is (<= (count thread-names) 6)
              (str "Expected ≤6 distinct stage-pool threads, got "
                   (count thread-names) ": " thread-names))
          ;; Sanity: thread names should belong to the stage pool.
          (is (every? #(re-find #"^chengis-parallel-stage-" %) thread-names)
              (str "Unexpected thread name(s) in stage pool: " thread-names))))
      (cleanup))))
