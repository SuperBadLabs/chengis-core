...
;; ---------------------------------------------------------------------------
;; MUT-P2 (Pareto): conversion fallbacks (matrix/strategy, string post keys,
;; exclude) and validation error messages that name the offender.
;; ---------------------------------------------------------------------------

(deftest build-pipeline-matrix-and-post-fallbacks-test
  (testing "matrix resolves from :strategy when there is no top-level :matrix"
    (let [p (#'yaml/build-pipeline-from-data {:stages [] :strategy {:matrix {:os ["linux" "mac"]}}})]
      (is (= ["linux" "mac"] (get-in p [:matrix :os])))))
  (testing "matrix :exclude resolves from :strategy and non-exclude keys are kept"
    (let [p (#'yaml/build-pipeline-from-data
             {:stages [] :strategy {:matrix {:os ["linux" "mac"]} :exclude [{:os "mac"}]}})]
      (is (= ["linux" "mac"] (get-in p [:matrix :os])))
      (is (seq (get-in p [:matrix :exclude])))))
  (testing "post actions accept string and underscore keys (always / on_success)"
    (let [p (#'yaml/build-pipeline-from-data
             {:stages [] :post {"always" [{:run "cleanup"}]
                                :on_success [{:run "notify"}]}})]
      (is (contains? (:post-actions p) :always))
      (is (contains? (:post-actions p) :on-success)))))

(deftest validate-error-messages-name-the-offender-test
  (testing "a named stage missing 'steps' is named in the error (kills or->and fallback)"
    (let [r (yaml/validate-yaml-workflow {:stages [{:name "build-stage"}]})]
      (is (false? (:valid? r)))
      (is (some #(clojure.string/includes? % "build-stage") (:errors r)))))
  (testing "a named step missing 'run' is named in the error"
    (let [r (yaml/validate-yaml-workflow
             {:stages [{:name "deploy-stage" :steps [{:name "step-a"}]}]})]
      (is (false? (:valid? r)))
      (is (some #(clojure.string/includes? % "step-a") (:errors r))))))
