  ...
  (testing "Registry can be initialized without errors"
    (let [registry (metrics/init-registry)]
      ...
      (let [dump (export/text-format registry)]
        ...
        ;; DB pool gauges are registered for PostgreSQL pool saturation visibility.
        ;; They may remain at 0 or absent from exposition until sampled.
        (is (re-find #"db_pool_active_connections" dump) "Should contain DB pool gauges")))))
...
(deftest metrics-handler-test
  (testing "Metrics handler returns Prometheus text format"
    (let [registry (metrics/init-registry)
          handler (metrics/metrics-handler registry nil)
...
(deftest record-db-pool-stats-non-hikari-noop-test
  (testing "record-db-pool-stats! is a no-op for nil/non-Hikari datasource"
    (let [registry (metrics/init-registry)]
      (is (nil? (metrics/record-db-pool-stats! registry nil)))
      (is (nil? (metrics/record-db-pool-stats! registry {}))))))
